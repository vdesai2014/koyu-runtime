...
class DataRecorder(Service):
    ...
    def __init__(self, home: str, sources: list[Source]):
        ...
        self._tele_fid = 0                             # telemetry frame_id: bumps on every
        self._tele_last = None                         # write so readers dedup on change
    ...
    def _telemetry(self) -> None:
        """Publish on change only, with a monotonic frame_id: consumers (bridge,
        ipc_logger) dedup on frame_id, so a constant one would freeze them after
        the first sample, and per-tick writes would spam their change logs."""
        now = (1 if self.state == "recording" else 0, len(self.rows), self.capture_id)
        if now == self._tele_last:
            return
        self._tele_last = now
        self._tele_fid += 1
        cell = types.RecorderTelemetry(
            timestamp=time.time(), frame_id=self._tele_fid,
            state=now[0], frames=now[1], capture_id=self.capture_id.encode())
