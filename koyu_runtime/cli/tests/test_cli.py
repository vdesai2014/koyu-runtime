...
from koyu_runtime.cli import main
...
def test_context_set_show_clear(tmp_path, capsys):
    rt = str(runtime_dir(tmp_path))
    rc = main.main(["context", "set", "task=pick", "source_run_id=run_x", "-r", rt])
    assert rc == 0
    out = json.loads(capsys.readouterr().out)
    assert out["task"] == "pick" and out["source_run_id"] == "run_x"

    rc = main.main(["context", "set", "task=", "-r", rt])     # key= clears one field
    assert rc == 0
    out = json.loads(capsys.readouterr().out)
    assert out["task"] is None and out["source_run_id"] == "run_x"   # others kept

    rc = main.main(["context", "-r", rt])                     # bare verb = show
    assert rc == 0
    assert json.loads(capsys.readouterr().out)["source_run_id"] == "run_x"

    rc = main.main(["context", "clear", "-r", rt])
    assert rc == 0
    assert not (tmp_path / "rt" / "recording-context.json").exists()


def test_context_rejects_unknown_key(tmp_path, capsys):
    rc = main.main(["context", "set", "vibe=good", "-r", str(runtime_dir(tmp_path))])
    assert rc == 2
